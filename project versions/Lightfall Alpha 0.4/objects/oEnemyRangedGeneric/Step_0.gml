...
switch (current_state)
{
	case enemy_states.patrol: {
		// patrol
		if ((grounded) && ( ((!place_meeting(x + hsp, y + 1,oWall) && afraid_of_heights)) or place_meeting(x+hsp,y, oWall) ))
		{
			hsp = -hsp;
		}
		x += hsp;
...
#region animations (turn to script later)
	if current_state = enemy_states.attack { //maybe check ranges instead if this doesn't respond well
		sprite_index = sCrowMage_Attack; //depend on timer + bullets
		image_xscale = sign(target.x-x);
	} else if current_state = enemy_states.patrol {
		if hsp !=0 sprite_index = sCrowMage_Walk; else sprite_index = sCrowMage_Idle;
	}
	if current_state = enemy_states.idle {
		sprite_index = sCrowMage_Idle;
		image_xscale = sign(target.x-x);
	}
